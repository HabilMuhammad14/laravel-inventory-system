...
<div class="sidebar" id="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="nav-item active" href="{{ route('transaksiRetur.index') }}">
        ...
        <tbody>
          <form action="{{route('transaksiReturController.store')}}" method="POST">
            ...
            <tr class="form-row">
              ...
              <td><input type="text" class="form-input" placeholder="RTR001" name="kode_transaksi_retur"></td>
          ...
          <form action="{{ route('transaksiRetur.update', $retur) }}" method="POST">
            ...
            <tr class="edit-row">
              <td style="color:#aaa;font-size:12px">{{ $loop->iteration }}</td>
                  ...
                  <td>
                    <input type="date" class="form-input {{ $errors->has('tanggal') ? 'input-error' : '' }}" name="tanggal" value="{{ old('tanggal', $retur->tanggal) }}">
                  ...
                  <td>
                    <select class="form-select {{ $errors->has('barang_id') ? 'input-error' : '' }}" name="barang_id">
                      @foreach($barangs as $barang)
                        <option value="{{ $barang->id }}" {{ old('barang_id', $retur->barang_id) == $barang->id ? 'selected' : '' }}>
                  ...
                  <td>
                    <input type="number" class="form-input {{ $errors->has('jumlah') ? 'input-error' : '' }}" name="jumlah" value="{{ old('jumlah', $retur->jumlah) }}" min="1">
                  ...
                  <td>
                    <select class="form-select {{ $errors->has('supplier_id') ? 'input-error' : '' }}" name="supplier_id">
                      @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id }}" {{ old('supplier_id', $retur->supplier_id) == $supplier->id ? 'selected' : '' }}>
                  ...
                  <td>
                    <select class="form-select {{ $errors->has('user_id') ? 'input-error' : '' }}" name="user_id">
                      @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $retur->user_id) == $user->id ? 'selected' : '' }}>
                          {{ $user->id }}
                        </option>
                      @endforeach
                    </select>
            ...
            @else
              <tr>
                ...
                <td>{{ $retur->tanggal }}</td>
                <td>{{ $retur->barang->kode_barang ?? '-' }}</td>
                <td>{{ $retur->jumlah }}</td>
                ...
                <td>
                  <div class="aksi-group">
                    ...
                    <a href="{{route('transaksiRetur.hapus', $retur)}}" class="btn-hapus" onclick="return confirm('Are you sure you want to delete this item?')">Hapus</a>
                  </div>
                </td>
              </tr>
            @endif
