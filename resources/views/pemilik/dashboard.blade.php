...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .card{background:#fff;border:1px solid #e5e5e5;border-radius:12px;padding:20px;text-decoration:none;color:#333;display:flex;flex-direction:column;gap:10px;transition:box-shadow .15s,border-color .15s}
    .card:hover{box-shadow:0 4px 16px rgba(0,0,0,.07);border-color:#c7d9f8}
    .card-icon{width:38px;height:38px;background:#dbeafe;border-radius:10px;display:flex;align-items:center;justify-content:center}
    .card-icon svg{width:18px;height:18px;stroke:#1a56db;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
    .card-label{font-size:13.5px;font-weight:600;color:#222}
    .card-desc{font-size:12px;color:#999;line-height:1.4}

    .section-title{font-size:14px;font-weight:600;color:#333;margin-bottom:12px}
    .table-card{background:#fff;border:1px solid #e5e5e5;border-radius:12px;overflow:hidden;overflow-x:auto}
    .tabel{width:100%;border-collapse:collapse;font-size:13px;min-width:400px}
    ...
    .badge-status{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11.5px;font-weight:500}
    .badge-status.menipis{background:#fef9c3;color:#854d0e}
    .badge-status.hampir-habis{background:#fee2e2;color:#b91c1c}
    .badge-status.habis{background:#f3f3f3;color:#888}
...
<div class="sidebar" id="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="nav-item active" href="{{route('pemilik.dashboard')}}">
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="page-sub">Ringkasan informasi inventaris toko.</div>
    ...
    <div class="grid">
      <a class="card" href="">
        ...
        <div class="card-label">Laporan Barang</div>
        <div class="card-desc">Melihat data seluruh barang toko</div>
      </a>
      <a class="card" href="">
        <div class="card-icon">
          <svg viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>
        </div>
        <div class="card-label">Laporan Transaksi</div>
        <div class="card-desc">Melihat aktivitas transaksi barang</div>
      </a>
      <a class="card" href="">
        <div class="card-icon">
          <svg viewBox="0 0 24 24"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
        </div>
        <div class="card-label">Laporan Stok</div>
        <div class="card-desc">Memantau kondisi stok barang</div>
      </a>
      <a class="card" href="">
        <div class="card-icon">
          <svg viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
        ...
        <div class="card-desc">Melihat data supplier toko</div>
      </a>
    </div>

    <div class="section-title">Barang Dengan Stok Menipis</div>
    <div class="table-card">
      <table class="tabel">
        ...
        <tbody>
          ...
          @php
            $status = $barang->stok <= 0 ? 'habis' : ($barang->stok <= 10 ? 'hampir-habis' : 'menipis');
            $label  = $barang->stok <= 0 ? 'Habis' : ($barang->stok <= 10 ? 'Hampir Habis' : 'Menipis');
          @endphp
          <tr>
          ...
          @empty
          <tr>
            <td colspan="5" style="text-align:center;color:#aaa;padding:20px">Semua stok barang dalam kondisi aman</td>
          </tr>
          @endforelse
        </tbody>
      </table>
