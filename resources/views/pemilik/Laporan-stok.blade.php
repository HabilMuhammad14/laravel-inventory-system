...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .tabel{width:100%;border-collapse:collapse;font-size:13px;min-width:700px}
    ...
    .badge-status.aman{background:#dcfce7;color:#16a34a}
...
<div class="sidebar" id="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="nav-item active" href="{{route('pemilik.laporan-stok')}}">
...
<div class="main">
  ...
  <div class="content">
    <div class="page-header">
      <div>
        <div class="page-title">Laporan Stok Barang</div>
        <div class="page-sub">Informasi stok barang masuk, keluar, dan sisa stok barang.</div>
      </div>
    ...
    <div class="summary-grid">
      ...
      <div class="summary-card green">
        <div class="summary-title">Barang Masuk</div>
        <div class="summary-value">{{ $totalMasuk }}</div>
      </div>
      <div class="summary-card red">
        <div class="summary-title">Barang Keluar</div>
        <div class="summary-value">{{ $totalKeluar }}</div>
      </div>
      <div class="summary-card orange">
        <div class="summary-title">Retur Barang</div>
        <div class="summary-value">{{ $totalRetur }}</div>
      </div>
      ...
      <table class="tabel">
        <thead>
          <tr>
            ...
            <th class="num" style="width:90px">Stok Awal</th>
            <th class="num" style="width:90px">Masuk</th>
            <th class="num" style="width:90px">Keluar</th>
            <th class="num" style="width:70px">Retur</th>
            <th class="num" style="width:90px">Stok Akhir</th>
        ...
        <tbody>
          ...
          @php
            $masuk     = $barang->transaksi_masuk_sum_jumlah ?? 0;
            $keluar    = $barang->transaksi_keluar_sum_jumlah ?? 0;
            $retur     = $barang->transaksi_retur_sum_jumlah ?? 0;
            $stokAkhir = $barang->stok;
            $stokAwal  = $stokAkhir + $keluar - $masuk + $retur;
            $statusClass = $stokAkhir <= 0 ? 'habis' : ($stokAkhir <= 20 ? 'menipis' : 'aman');
            $statusLabel = $stokAkhir <= 0 ? 'Habis' : ($stokAkhir <= 20 ? 'Menipis' : 'Aman');
          @endphp
          <tr>
            <td style="color:#aaa;font-size:12px">{{ $loop->iteration }}</td>
            <td><span class="badge-id">{{ $barang->kode_barang }}</span></td>
            <td>{{ $barang->nama_barang }}</td>
            <td class="num">{{ $stokAwal }}</td>
            <td class="num" style="color:#16a34a">{{ $masuk }}</td>
            <td class="num" style="color:#b91c1c">{{ $keluar }}</td>
            <td class="num" style="color:#d97706">{{ $retur }}</td>
            <td class="num"><strong>{{ $stokAkhir }}</strong></td>
